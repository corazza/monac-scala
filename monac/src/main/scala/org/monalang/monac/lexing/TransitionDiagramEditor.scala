...
object TransitionDiagramEditor {
    ...
    def getEtaClosureInner(acc: List[Int], nfaState: Int): List[Int] = {
      ...
      if (first.nonEmpty) {
        val others = first map { state =>
          if (acc.contains(state)) List[Int]()
  ...
  private def expandState(on: TransitionDiagram, state: Int, out: TransitionDiagram) {
    ...
    for (i <- 0 until out.currentStates) {
      on.addTransition(state, myState(i), out.matrix.head(i))
    }
  ...
  private def eval(node: Node): TransitionDiagram = node match {
    ...
    case Id => {
      val result = TransitionDiagram(2)
      result.addTransition(0, 1, TransitionDiagram.IdTransition)
      result
    }

    case NonId => {
      val result = TransitionDiagram(2)
      result.addTransition(0, 1, TransitionDiagram.NonIdTransition)
      result
    }

    case Unicode => {
      val result = TransitionDiagram(2)
      result.addTransition(0, 1, TransitionDiagram.UnicodeTransition)
      result
    }
